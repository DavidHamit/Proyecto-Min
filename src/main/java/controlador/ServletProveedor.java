	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(request.getParameter("btncre")!=null) {
			...
			if(nom.equals("") || dir.equals("") || tel==0 || ciu.equals("")){
				...
				response.sendRedirect("proveedores.jsp?mensaje="+mensaje);
			}
			else if(nit==0){
				//JOptionPane.showMessageDialog(null, "Falta NIT del Proveedor");
				mensaje="Falta NIT del Proveedor";
				response.sendRedirect("proveedores.jsp?mensaje="+mensaje);
				}
			else {
		...
		if(request.getParameter("btnbor")!=null) {
			...
			if(q.equals("q")) {
				...
				mensaje="Proveedor con NIT "+nit+", han sido borrados";
				response.sendRedirect("proveedores.jsp?mensaje="+mensaje);
			}
			else if(q.equals("n")){
		...
		if(request.getParameter("btnact")!=null) {
			...
			if(nom.equals("") || dir.equals("") || tel==0 || ciu.equals("")) {
				...
				if(p) {
					...
					mensaje="Los datos del Proveedor han sido actualizados";
					response.sendRedirect("proveedores.jsp?mensaje="+mensaje);
				}
				else {
					//JOptionPane.showMessageDialog(null, "Proveedor NO existe en la base de datos");
		...
		if(request.getParameter("btncon")!=null) {
			...
			if(pcons==null) {
				//JOptionPane.showMessageDialog(null, "Proveedor NO existe en la base de datos");
				mensaje="Proveedor NO existe en la base de datos";
				response.sendRedirect("proveedores.jps?mensaje="+mensaje);
			}
			else{
				ni=pcons.getNit();
				nom=pcons.getNom();
